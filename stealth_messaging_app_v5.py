...
def kill_all_sessions():
    """Kill all active sessions (demigod command)."""
...
def reset_global_kill_switch():
    """Reset the global kill switch (allows new sessions)."""
...
def cloak_session(session_url):
    """Cloak a session (hide from new users)."""
    cloaked_sessions.add(session_url)
    print(f"[INFO] Session {session_url} has been cloaked")

def uncloak_session(session_url):
    """Uncloak a session (make visible to new users)."""
    cloaked_sessions.discard(session_url)
    print(f"[INFO] Session {session_url} has been uncloaked")
...
def set_impersonation(username, fake_name):
    """Set impersonation for a user."""
    if fake_name:
        user_impersonations[username] = fake_name
        print(f"[INFO] User {username} is now impersonating as {fake_name}")
    else:
        user_impersonations.pop(username, None)
        print(f"[INFO] User {username} stopped impersonating")
...
def login():
    ...
    if user_data and bcrypt.checkpw(password.encode("utf-8"), user_data["password"].encode("utf-8")):
        ...
        session_urls[chat_url_component] = username
    ...
    <body>
        ...
        <script>
            ...
            async function sendMessage() {
                ...
                try {
                    ...
                    if (response.ok && data.success) {
                        ...
                        // No need to manually load messages - WebSocket will handle it
                    } else {
            ...
            function appendMessage(msg) {
                ...
                if (msg.sender === currentUsername || currentUsername === 'demigod') {
...
def send_message():
    ...
    # Check if global kill switch is active
    if is_global_session_killed():
        # Check for revive command
        if message_text and message_text.strip() == "/revive" and sender == "demigod":
            reset_global_kill_switch()
            return jsonify({"success": True, "message": "Global session restored by demigod."})
        return jsonify({"success": False, "message": "All sessions have been terminated."}), 403
    ...
    # Check for admin commands (only for demigod user)
    if message_text and sender == "demigod":
        if message_text.strip() == "/nuke":
            kill_all_sessions()
            return jsonify({"success": True, "message": "All sessions terminated by demigod."})
        elif message_text.strip() == "/cloak":
            cloak_session(current_session_url)
            return jsonify({"success": True, "message": "Session cloaked successfully."})
        elif message_text.strip() == "/uncloak":
            uncloak_session(current_session_url)
            return jsonify({"success": True, "message": "Session uncloaked successfully."})
        elif message_text.strip().startswith("/impersonate "):
            fake_name = message_text.strip()[13:].strip()  # Remove "/impersonate " prefix
            if fake_name:
                set_impersonation(sender, fake_name)
                return jsonify({"success": True, "message": f"Now impersonating as {fake_name}."})
            else:
                set_impersonation(sender, None)
                return jsonify({"success": True, "message": "Stopped impersonating."})
...
def delete_message():
    ...
    for i, msg in enumerate(messages):
        if msg["id"] == message_id:
            ...
            real_sender = sender
            display_name = get_display_name(sender)
            if msg["sender"] == display_name or sender == "demigod":
                del messages[i]
                message_found = True
...
if __name__ == "__main__":
    print("[INFO] Starting Stealth Messaging App Enhanced v6...")
    print("[INFO] Access via: http://localhost:5001/?user=demigod or http://localhost:5001/?user=human")
    ...
    print("[INFO] Admin commands: /nuke (kill all), /cloak (hide session), /uncloak (show session), /impersonate [name] (change display name)")
