...
# Fixed SocketIO configuration for production deployment
...
def chat_session(session_url):
    ...
    <body>
        ...
        <script>
            ...
            // Fixed audio initialization for production
            function initializeAudio() {
                try {
                    if (!window.alertSound) {
                        window.alertSound = new Audio();
                        // Using a short beep sound for better compatibility
                        ...
                        if (playPromise !== undefined) {
                            playPromise.then(() => {
                                ...
                                window.audioInitialized = true; // Still mark as initialized
                            });
                        } else {
                            window.audioInitialized = true;
                        }
            ...
            function playAlertSound() {
                if (!soundsMuted && window.alertSound && window.audioInitialized && userHasInteracted) {
                    try {
                        window.alertSound.currentTime = 0;
                        const playPromise = window.alertSound.play();
                        if (playPromise !== undefined) {
            ...
            // Fixed SocketIO initialization with better error handling
            ...
            function appendMessage(msg) {
                ...
                if (msg.alert) {
                    ...
                    // Play alert sound for alert messages
                    playAlertSound();
                }
            ...
            function updateMuteStatus(username) {
                fetch(`{{ url_for('check_mute_status') }}?username=${encodeURIComponent(username)}`)
...
if __name__ == "__main__":
    ...
    print("[INFO] Admin commands: /nuke, /revive, /cloak, /uncloak, /impersonate, /help, /status, /mute")
